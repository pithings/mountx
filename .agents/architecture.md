...
| `handles.ts`                        | `FileHandleTable` (ids off a monotonic counter, so a dropped entry still answers `ESTALE`; `maxHandles` evicts LRU past a cap, root exempt) and `DirectorySnapshots`                                                           |
