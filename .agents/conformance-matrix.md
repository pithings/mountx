...
| Case                                                                 | Needs      | loopback                                                                       | FUSE | NFS  | S3             |
| -------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------ | ---- | ---- | -------------- |
...
| rejects an exclusive open of a symlink, dangling or not, with EEXIST | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
...
| reports filesystem statistics            | `statfs` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: statfs |
...
| keeps an open handle readable after unlink                     | `handles` | pass     | pass | skip: handles | pass |
...
| Case                                                                    | Needs       | loopback                                                                       | FUSE | NFS  | S3              |
| ----------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------ | ---- | ---- | --------------- |
| shares an inode and counts nlink                                        | `hardlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: hardlinks |
| rejects an existing destination with EEXIST                             | `hardlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: hardlinks |
| rejects linking a directory with EPERM and a missing source with ENOENT | `hardlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: hardlinks |

### symlinks

| Case                                                           | Needs      | loopback                                                                       | FUSE | NFS  | S3             |
| -------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------ | ---- | ---- | -------------- |
| round-trips through readlink and distinguishes lstat from stat | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
| resolves symlinked directories as path components              | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
| keeps dangling symlinks visible to lstat only                  | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
| rejects symlink loops with ELOOP                               | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
| rejects readlink of a regular file with EINVAL                 | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
| sizes a symlink by its target in bytes                         | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |
| rejects creating a symlink over an existing name with EEXIST   | `symlinks` | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass | skip: symlinks |

### metadata

| Case                                                                   | Needs                               | loopback                                                                       | FUSE | NFS        | S3                                  |
| ---------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------ | ---- | ---------- | ----------------------------------- |
| changes permission bits                                                | `permissions`                       | pass                                                                           | pass | pass       | skip: permissions                   |
| keeps ownership when set to the current owner                          | `permissions`                       | pass                                                                           | pass | pass       | skip: permissions                   |
| sets access and modification times                                     | `times`                             | pass                                                                           | pass | pass       | skip: times                         |
| sets times on the symlink itself with lutimes                          | `times` + `symlinks`                | pass (memory), pass (node-fs), skip (unstorage), pass (node:fs/promises (raw)) | pass | pass       | skip: times + symlinks              |
| changes ownership of the symlink itself with lchown, not of its target | `permissions` + `symlinks` + `root` | skip: root                                                                     | pass | skip: root | skip: permissions + symlinks + root |
