...
4. **FUSE transport (root mode).** Open `/dev/fuse`, spawn
...
5. **Validation.** Differential suite: same op sequence against passthrough
   mount vs real `node:fs`, diff results. `unimount record`/replay fixtures.
   pjdfstest run against a real mount (locally under sudo and in a CI job).
