...
- **Not able to create special files through `node:fs` alone.** FIFOs, sockets and device nodes need [`mountx.mknod`](/reference/capabilities#extensions), because `node:fs/promises` cannot create them — the [memory driver](/guide/drivers/built-in#special-files) implements it, a passthrough to the host cannot.
