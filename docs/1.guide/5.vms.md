...
- **The guest is a second client.** Both servers accept several connections. 9P shares one byte-range lock table across them, so a lock taken in the guest is seen on the host — but renames are still only serialised per connection, so mounting one driver from the host _and_ a guest at the same time is [worth reading first](/transports/9p#serving-more-than-one-client).
