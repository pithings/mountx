...
Three ship with the package, so you may not have to write one at all:
...
- [**Memory**](/reference/drivers#memory) — files, directories, symlinks, hardlinks, permissions, timestamps and `statfs`, living as long as the process. The default for tests and scratch trees.
- [**Node-fs**](/reference/drivers#node-fs) — a real directory served back through a mount, with every path component resolved inside its root so nothing escapes. The thing to wrap when you want caching, logging or filtering over real files.
- [**Unstorage**](/reference/drivers#unstorage) — an [unstorage](https://unstorage.unjs.io) `Storage` as a tree, which makes S3, Redis, GitHub and every other unstorage driver mountable. A key–value store has no partial reads, no symlinks and no atomic rename, and the driver declares all of it.

Options, trade-offs and how to wrap one are in the [drivers reference](/reference/drivers). The rest of this page is about writing your own.
...
The optional methods are the rest of `fs/promises`: `lstat`, `statfs`, `mkdir`, `rmdir`, `unlink`, `rename`, `link`, `symlink`, `readlink`, `chmod`, `chown`, `lchown`, `truncate`, `utimes`, `lutimes`. Full signatures in the [reference](/reference/mountx#fsdriver).
...
- [`mountx` reference](/reference/mountx) — every type on this page in full.
- [Bundled drivers](/reference/drivers) — memory, node-fs and unstorage, and what they are good for.
