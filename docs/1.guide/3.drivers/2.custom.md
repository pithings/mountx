...
`test/conformance.ts` in the repository is one suite written against the driver interface, and it is run four ways: through the loopback harness, through a real FUSE mount, through a real NFSv3 socket, and through a real NFSv4.1 socket. So a test that passes in one column and fails in another is a transport bug **by construction**, not a driver bug.
