...
It is also the driver that implements [`mountx.mknod`](/reference/capabilities#extensions), so **FIFOs, UNIX sockets and device nodes are in-memory objects here like any other** — see below.
...
`mknod` takes the four types `node:fs/promises` cannot create — FIFO, socket, character device, block device — plus a regular file, for the callers that still create an empty one this way. The mode carries the type; `dev` is the device number, and is ignored for a FIFO and a socket.

There is no kernel obstacle to any of it: the node carries the type and the device number, and the client's own VFS supplies the pipe, socket and device semantics. So a FIFO created this way and mounted is a **real FIFO** — `mkfifo` on the mount works, and so does a program that opens both ends of it.

Two things follow from the node holding no bytes of its own, and both are deliberate:

- **`open()` and `truncate()` on one answer `ENXIO` and `EINVAL`.** Nothing reaches them over a mount — every client handles these types locally and never sends the open across — so the only caller who can is a loopback one, and a byte buffer would be a filesystem no mount has.
