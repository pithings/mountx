...
- **No handle-table leak.** NFSv3's handle table only grows (there is no `FORGET`); 9P's fid table is bounded by what the client currently has open, and a clunked fid is actually released.
...
| `aname`            | `"/"`                             | the tree to attach to; `""` (the kernel's own `V9FS_DEFANAME`) also works — anything else is `ENOENT`, there is no subtree export |
...
The same discipline as [FUSE](/transports/fuse#teardown-and-what-it-cant-do) and [NFS](/transports/nfs#teardown): `umount(8)` first, the mount table as the truth rather than an exit status, forcing down on the deadline, idempotent and retryable throughout. `umount -f` is not a placebo here — v9fs implements `.umount_begin` (`v9fs_umount_begin()`), which cancels requests in flight, the same role `fuse_abort_conn` plays for FUSE.
