...
**One driver interface, three ways of getting it in front of a kernel.**

A transport is everything between the driver you wrote and the operating system: the protocol codec, the session that turns messages into driver calls, and the piece that attaches the result to a directory. `mountx/auto` picks one for you; all three can be [pinned](#pinning-a-transport) directly.
