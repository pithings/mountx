...
| ---------------------------------------------------------- | -------------------------------------------------------------------- |
| [`mountx/auto`](/reference/auto)                           | **start here** — `mount()`, which picks the transport for this host  |
| [`mountx`](/reference/mountx)                              | the driver types, errors, path helpers, loopback harness             |
| [`mountx/drivers/memory`](/reference/drivers#memory)       | a ready-made in-memory filesystem                                    |
| [`mountx/drivers/node-fs`](/reference/drivers#node-fs)     | a ready-made passthrough to a real folder                            |
| [`mountx/drivers/unstorage`](/reference/drivers#unstorage) | an adapter over any [unstorage](https://unstorage.unjs.io) `Storage` |
| [`mountx/fuse`](/reference/fuse)                           | the FUSE transport on its own, if you want to pin it                 |
| [`mountx/nfs`](/reference/nfs)                             | the NFSv3 transport, and `createNfsServer()`                         |

Plus the [`mountx` CLI](/reference/cli), which the package installs as a binary.
