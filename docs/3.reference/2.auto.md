...
See [`MountOptions`](/reference/fuse#mountoptions) and [`MountNfsOptions`](/reference/nfs#mountnfsoptions) for the two nested blocks.
...
## The three deliberate limits

- **No fallback after a failure.** The probe decides once, from host facts. If the chosen transport then fails to mount, that error is what you get.
- **No probing when a transport is named.** `transport: "fuse"` calls the FUSE transport directly, whose own errors are more specific.
- **No loading of what it does not use.** Both transports arrive through `await import()`.
