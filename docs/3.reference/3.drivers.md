...
- **No symlinks, no hardlinks, no `statfs`**, and `rename` is a copy followed by a delete rather than an atomic operation. All four are declared, so the mount answers `ENOSYS`/`ENOTSUP` rather than pretending — see [capabilities](/guide/drivers#capabilities).
