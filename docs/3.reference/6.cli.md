...
Seeds an in-memory filesystem with a file or two, wraps it in a request logger, and mounts it through [`mountx/auto`](/reference/auto) — so it picks FUSE on Linux and NFSv3 on macOS. Then it narrates: every `lookup`, `getattr`, `open`, `read` and `write` the kernel sends, as it happens.
